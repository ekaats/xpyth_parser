...
class XPath_parser_tests(unittest.TestCase):
    ...
    def test_kind_tests(self):
        # Empty document-node
        self.assertEqual(list(t_KindTest.parseString(f"document-node()", parseAll=True)),
                         [Test(test_type="document-node")])

        # Empty element test
        self.assertEqual(list(t_KindTest.parseString(f"element()", parseAll=True)), [Test(test_type="element")])

        # document-node with element test
        self.assertEqual(list(t_KindTest.parseString(f"document-node(element())", parseAll=True)),
                         [Test(test_type="document-node", test=Test(test_type="element"))])

        # document-node with element test and a Qname
        self.assertEqual(list(t_KindTest.parseString(f"document-node(element(test:element))", parseAll=True)),
                         [Test(test_type="document-node",
                               test=Test(test_type="element", test=QName(prefix="test", localname="element")))])

        # document-node with schema element test and a Qname
        self.assertEqual(list(t_KindTest.parseString(f"document-node(schema-element(element))", parseAll=True)),
                         [Test(test_type="document-node",
                               test=Test(test_type="schema-element", test=QName(localname="element")))])

        # Empty attribute test
        self.assertEqual(list(t_KindTest.parseString(f"attribute()", parseAll=True)), [f"attribute"])

        # Schema-element test
        self.assertEqual(list(t_KindTest.parseString(f"schema-element(px:name)", parseAll=True)),
                         [Test(test_type="schema-element", test=QName(prefix="px", localname='name'))])

        # Schema-element test
        self.assertEqual(list(t_KindTest.parseString(f"schema-attribute(px:name)", parseAll=True)),
                         [Test(test_type="schema-attribute", test=QName(prefix="px", localname='name'))])

        # empty processing-instruction test
        self.assertEqual(list(t_KindTest.parseString(f"processing-instruction()", parseAll=True)),
                         [Test(test_type="processing-instruction")])

        self.assertEqual(list(t_KindTest.parseString(f"processing-instruction(name)", parseAll=True)),
